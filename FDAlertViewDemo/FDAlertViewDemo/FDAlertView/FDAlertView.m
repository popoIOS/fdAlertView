...
- (instancetype)initWithTitle:(NSString *)title icon:(UIImage *)icon message:(NSString *)message buttonTitles:(NSArray *)buttonTitles{
    if (self = [super initWithFrame:[UIScreen mainScreen].bounds]) {
        _icon = icon;
        _title = title;
        _message = message;
        _buttonArray = [NSMutableArray array];
        _buttonTitleArray = [NSMutableArray arrayWithArray:buttonTitles];
...
- (void)initAllButtons {
    if (_buttonTitleArray.count > 0) {
        ...
        UIView *horizonSperatorView = [[UIView alloc] initWithFrame:CGRectMake(0, _messageLabel.frame.origin.y + _messageLabel.frame.size.height + SPACE_LARGE, contentViewWidth, 0.6)];
        ...
        for (NSString *buttonTitle in _buttonTitleArray) {
            ...
            if (index < _buttonTitleArray.count - 1) {
                UIView *verticalSeperatorView = [[UIView alloc] initWithFrame:CGRectMake(button.frame.origin.x + button.frame.size.width, button.frame.origin.y, 0.6, button.frame.size.height)];
